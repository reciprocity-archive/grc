...
co = Section.create(:title => 'CO 1', :slug => 'REG1-CO1', :description => 'x', :program => prog)
ctl = Control.create(:title => 'Control 1', :slug => 'REG1-CTL1', :description => 'x', :program => prog, :is_key => true, :fraud_related => false)
(2..SIZE).each do |ind|
  Control.create(:title => "Control #{ind}", :slug => "REG2-CTL#{ind}")
end
company_ctl = Control.create(:title => 'Company Control 1', :slug => 'COM-CTL1', :description => 'x', :program => prog, :is_key => true, :fraud_related => false)
cycle = Cycle.create(:program_id => prog, :start_at => '2011-01-01', :complete => false)
...
sys.owner = person2
sys.save
sc = SystemControl.create(:control_id => ctl, :system_id => sys, :cycle_id => cycle)
...
sys_person1 = SystemPerson.create(:person_id => person1, :system_id => sys)
bp_person1 = BizProcessPerson.create(:person_id => person1, :biz_process_id => bp)
