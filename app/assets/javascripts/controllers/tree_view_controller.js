...
CMS.Controllers.Filterable("CMS.Controllers.TreeView", {
  ...
  fetch_list: function() {
    ...
    this.options.find_params.serialize(), this.proxy("draw_list"));
  },
  ...
  ".link-object modal:success": function(el, ev, data) {
    ev.stopPropagation();
    this.link_objects(
    el.data("child-type"), el.data("child-property"), el.closest("[data-object-id]"), data);
  }

  ,
  " linkObject": function(el, ev, data) {
    this.link_objects(
    data["childType"], data["childProperty"], this.element.children("[data-object-id=" + data["parentId"] + "]"), data.data);
  }
  ...
  link_objects: function(child_object_type, child_property, $parent, data) {
    var that = this,
      ...
      $list = parentid ? $parent.find(".item-content:first").children(".tree-structure[data-object-type=" + child_object_type + "]") : null,
      existing = parentid ? $list.children("[data-object-id]").map(function() {
    ...
    can.each(id_list, function(v) {
      ...
      if (!~can.inArray(v, existing)) {
        find_dfds.push(child_options.model.findOne({
          id: v
        }));
      }
    })
